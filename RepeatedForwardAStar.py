...
class RepeatedForwardAStar:
    def __init__(self, mazeFile, tie_break ='small'):
        ...
        self.fig.canvas.manager.set_window_title('Repeated Forward A* Maze Solver')
    ...
    def drawMaze(self, current_pos=None, path=None, delay=0.05):
        ...
        self.ax.set_xticks([])
        self.ax.set_yticks([])
        self.ax.set_title(f'Repeated Forward A* - Replans: {self.replans} - Expanded: {self.cells_expanded}')
    ...
    def senseNeighbors(self, x, y):
        ...
        for nx, ny in self.getNeighbors(x, y):
            ...
            if self.knowledge[nx][ny] == 'U':
                ...
                #print(f"Updated knowledge grid at {nx, ny}: {self.knowledge[nx][ny]}")    
                has_updated = True
        return has_updated
    ...
    def search(self):
        ...
        while current_pos != self.goal:
            ...
            if self.senseNeighbors(*current_pos):
                #print("Knowledge updated, replanning...")
    ...
    def planPath(self, start):
        ...
        # Choose a constant c (larger than the largest g(s) in your grid)
        c = 5000  # Example constant, adjust as needed

        # Insert start node with adjusted priority based on hint
        # Breaking ties for larger g-values
        # open_heap.insert((c * f_scores[start] - g_scores[start], start))
        # Breaking ties for smaller g-values
...
if __name__ == '__main__':
    ...
    rfAStar = RepeatedForwardAStar(mazeFile)
    path = rfAStar.search()
